...
{
  Matrix mat;
  mat.setZero(2*no_parts,2*no_parts);
  Matrix core_ham_mo(no_parts,no_parts);
...
Eigen::Tensor<double, 4> Hfock::initial_amps(Matrix &mat, Eigen::Tensor<double,4> &ten)
{
  ...
  Eigen::Tensor<double,4> t(2*no_occ,2*no_occ,2*no_vir,2*no_vir);
  double Emp2=0;
...
//Initializing a tensor to store the spin orbital basis 2-electron integrals
  spin_eri = Eigen::Tensor<double,4> (2*no_parts,2*no_parts,2*no_parts,2*no_parts);
...
//Creating initial T1s and T2s
  ...
  t_ia = Matrix::Zero(2*no_occ, 2*no_vir);
  //cout<<t_ia<<std::endl;
