...
int Hfock::update_fock_list(int fock_count)
{
    Matrix mat;
    Matrix mat2;
    mat.setZero(no_parts, no_parts);
    mat2.setZero(no_parts, no_parts);
    //cout<<"FDS"<<(fock * density * overlap)<<std::endl;
    //cout<<"SDF"<<(overlap * density * fock)<<std::endl;
    mat = fock * density * overlap;
    mat2 = overlap * density * fock;
    mat = mat - mat2;
    mat = ortho.transpose() * mat * ortho; 
     if(fock_count >= 5)
     {
         for(int i=0; i < no_parts; i++)
         {
             for(int j=0; j < no_parts; j++)
             {
                 for(int n=0; n < 5; n++)
                 {
                    fock_list(n, i, j) = fock_list(n+1, i, j);
                    err_list(n, i, j) = err_list(n+1, i, j);
                 }
                 fock_list(5, i, j) = fock(i, j);
                 err_list(5, i, j) = mat(i, j);
             }
         }
         fock_count = 5;
     }
     else
     {
         for(int i=0; i < no_parts; i++)
         {
             for(int j=0; j < no_parts; j++)
             {
                 fock_list(fock_count+1, i, j) = fock(i,j);
                 err_list(fock_count+1, i, j) = mat(i, j);
             }
         }
         fock_count++;
     }
...
bool Hfock::converge_err(int fock_count)
{
    bool conv_err = false;
    if(fock_count == 0)
        conv_err = true;
    for(int n=0; n < fock_count; n++)
    {
        for(int m=0; m < fock_count; m++)
        {
            if(std::abs(B(n,m)) > std::pow(10,-12))
            {
                conv_err = true;
            }
        }
...
//building the new fock matrix
  ...
  cout<<"\nInitial Fock matrix: \n"<< fock<<'\n';
  fock_list.resize(6, no_parts, no_parts);
  err_list.resize(6, no_parts, no_parts);
  Matrix mat;
  mat = fock * density * overlap - overlap * density * fock;
  for(int i=0; i < no_parts; i++)
  {
      for(int j=0; j < no_parts; j++)
      {
          fock_list(0, i, j) = fock(i,j);
          err_list(0, i, j) = mat(i,j);
      }
  }
  int fock_count = 0;
...
//SCF procedure
  ...
  {
     make_fock(density, fock);
//Fock count = index of the latest Fock matrix added to the list
//Updating the list of fock matrices and the list of error matrices
     fock_count = update_fock_list(fock_count);
//Begin DIIS procedure
     if(count >= 4)
     {
//Computing B here, then solving the system of linear equations for coeffs
         B.setZero(fock_count+2, fock_count+2);
         for(int n=0; n < fock_count+1; n++)
         {
             for(int m=0; m < fock_count+1; m++)
             {
                 B(n,m) =0;
                 for(int i=0; i < no_parts; i++)
                 {
                     for(int j=0; j < no_parts; j++)
                     {
                         B(n,m) += err_list(n, i, j)*err_list(m, i, j);
                     }
                 }
             }
             B(fock_count+1, n) = -1.0;
             B(n, fock_count+1) = -1.0;
         }
         C.setZero(fock_count+2); 
         C(fock_count+1) = -1.0;
         cout<<"B matrix\n"<<B<<std::endl;
         cout<<"C vector\n"<<C<<std::endl;
         Eigen::VectorXd X;
//Using Householder QR to solve the linear equations
         Eigen::ColPivHouseholderQR<Matrix> dec(B);
         X = dec.solve(C);
         cout<<"X vector\n"<<X<<std::endl;
         fock.setZero(no_parts, no_parts);

//Creating the new Fock matrix as a linear combination of previous Fock matrices
         for(int i=0; i < no_parts; i++)
         {
             for(int j=0; j < no_parts; j++)
             {
     ...
     old_density = density;
     make_density(ortho,fock,newfock,density);
     //cout<<"Trace of density: "<<density.trace();
     olde=newe;
     newe=scf_energy(density, fock);
     printf("Iteration number: %i Total SCF energy: %8.12f \n", count, newe);
     count++;
  }
